...
public abstract class Exercise implements Comparable<Exercise> {
	...
	public String toString() {
		return String.format("%s\t%s\t%s\t%.2f\t%s\t%.2f\t%s", name,getType(),getDateAsString(),duration,toStringCustomInfo(),comment);
	}
	...
	public int compareTo(Exercise other) {
		double calories2 = getCaloriesBurned();
		double  calories1 = getCaloriesBurned();
	...
	public String toSummaryString() {
		return String.format("%-20s%-15s%-15.2f%-15s", name, df.format(date), duration, comment);
	}
}
